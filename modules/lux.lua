...
---------------------------------------------------------------------
RegisterNetEvent("hcyk_hud:TogDfltSrnMuted_c")
AddEventHandler("hcyk_hud:TogDfltSrnMuted_c", function(sender, toggle)
...
---------------------------------------------------------------------
RegisterNetEvent("hcyk_hud:SetLxSirenState_c")
AddEventHandler("hcyk_hud:SetLxSirenState_c", function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent("hcyk_hud:TogPwrcallState_c")
AddEventHandler("hcyk_hud:TogPwrcallState_c", function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent("hcyk_hud:SetAirManuState_c")
AddEventHandler("hcyk_hud:SetAirManuState_c", function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if count_bcast_timer > delay_bcast_timer then
						...
						--- IS EMERG VEHICLE ---
						if GetVehicleClass(veh) == 18 then
							TriggerServerEvent("hcyk_hud:TogDfltSrnMuted_s", dsrn_mute)
							TriggerServerEvent("hcyk_hud:SetLxSirenState_s", state_lxsiren[veh])
							TriggerServerEvent("hcyk_hud:TogPwrcallState_s", state_pwrcall[veh])
							TriggerServerEvent("hcyk_hud:SetAirManuState_s", state_airmanu[veh])
						end
					else
...
-- Nahradit aktuální implementaci na konci souboru touto přesnější verzí:

function GetSirenState()
    ...
    -- Kontrola třídy vozidla - musí být emergency (18)
    if GetVehicleClass(veh) ~= 18 then
        return 0
    end
    
    -- Inicializace stavu pro toto vozidlo, pokud neexistuje
    if state_lxsiren == nil then
        state_lxsiren = {}
    end
    ...
    -- Kontrola, zda jsou zapnuté majáky
    if IsVehicleSirenOn(veh) then
        -- Pokud je aktivní nějaký tón sirény, vrátit stav 2 nebo výše (siréna se zvukem)
        if state_lxsiren[veh] > 0 then
            return 2  -- Vrátit 2 pro sirénu se zvukem (bez ohledu na konkrétní tón)
        else
            return 1  -- Jen světla bez zvuku
        end
    else
        return 0  -- Siréna zcela vypnutá
    end
end

exports('GetSirenState', GetSirenState)